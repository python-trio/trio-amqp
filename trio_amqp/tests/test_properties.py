...
class TestReply(testcase.RabbitTestCase):
    async def _server(self, amqp, server_future, exchange_name, routing_key, task_status=trio.TASK_STATUS_IGNORED):
    ...
    async def _client(
            self, amqp, client_future, exchange_name, server_routing_key, correlation_id,
            client_routing_key, task_status=trio.TASK_STATUS_IGNORED):
    ...
    async def test_reply_to(self, amqp):
        ...
        async with trio.open_nursery() as n:
            ...
            await n.start(self._client,
                amqp, client_future, exchange_name, server_routing_key, correlation_id, client_routing_key)
